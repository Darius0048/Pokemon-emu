...
import { mockPlayers } from "./mock/mockData";
...
const PokemonEmulatorApp = () => {
  ...
  const handleRomLoad = (rom) => {
    setRomFile(rom);
  };

  const handleJoinRoom = (roomId, hostStatus) => {
    ...
    if (roomId) {
      ...
      setTimeout(() => {
        if (hostStatus) {
          setConnectedPlayers([
            { name: 'You', isHost: true, status: 'ready' }
          ]);
        } else {
          setConnectedPlayers([
            { name: 'Host Player', isHost: true, status: 'ready' },
            { name: 'You', isHost: false, status: 'connected' }
          ]);
        }
      }, 500);
      
      // Mock second player joining after some time
      if (hostStatus) {
        setTimeout(() => {
          setConnectedPlayers([
            { name: 'You', isHost: true, status: 'ready' },
            { name: 'Friend', isHost: false, status: 'connected' }
          ]);
        }, 3000);
      }
    } else {
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <RomUploader onRomLoad={handleRomLoad} currentRom={romFile} />
            <RoomSystem 
